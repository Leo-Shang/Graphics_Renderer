...
public class ApplyViewToScreen {
    private static double[][] view_to_screen_matrix = new double[4][4];

    public static Polygon multiplyViewToScreenMatrix(Polygon vertices){
        view_to_screen_matrix = SimpInterpreter.getViewToScreen();
        ...
        for(int index = 0; index < vertices.length(); index++){
            ...
            for(int i = 0; i < 4; i++) {
                double lineTotal = 0;
                for(int j = 0; j < 4; j++) {
                    lineTotal += view_to_screen_matrix[i][j] * current_point[j];
                }
                temp[i] = lineTotal;
            }
            current_point = temp;
